...
// IssuerServer exposes all dependencies needed to run a http server and its services
...
// NewIssuerServer does two things: instantiates all service and registers their HTTP bindings
func NewIssuerServer(shutdown chan os.Signal, cfg config.ServiceConfig) (*IssuerServer, error) {
	...
	ssi, err := service.InstantiateService(cfg.Services)
	if err != nil {
