...
async function createContextInstance(address: string, signer: HardhatEthersSigner) {
  const contextHandler = AccessContextHandler__factory.connect(address, signer);
  const user = `did:pkh:eip155:${signer.address}`;
...
async function setupRole(address: string, signer: HardhatEthersSigner, policyInstance: string) {
...
task("register-policy", "Register a sample policy", async (_, hre) => {
  ...
  const createInstanceReceipt = await createContextInstance(contextHandlerAddress, signer);
  ...
  await setupRole(contextHandlerAddress, signer, policy.address);
  console.log("setup role receipt");
});
